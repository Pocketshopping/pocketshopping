...
class LogisticTracker extends StatefulWidget {
  LogisticTracker({this.order, this.user});

  final String order;
  final User user;

  @override
  State<StatefulWidget> createState() => _LogisticTrackerState();
}

class _LogisticTrackerState extends State<LogisticTracker> {
                                                              ...
                                                              Expanded(
                                                                child: FlatButton.icon(
                                                                    onPressed: (){
                                                                      ...
                                                                      Get.to(
                                                                          CustomerMapTracker(
                                                                            latLng: LatLng(order.data.orderMode.coordinate.latitude,
                                                                              order.data.orderMode.coordinate.longitude,),
                                                                            customerName: order.data.orderCustomer.customerName,
                                                                            telephone: order.data.orderCustomer.customerTelephone
                                                                            ,)
                                                                      );
                                                                    },
                                                                    color: Colors.green,
                                                                ...
                                                                child: FlatButton.icon(
                                                                    onPressed: (){
                                                                      Get.to(
                                                                          ...
                                                                          CustomerMapTracker(
                                                                            latLng: LatLng(merchant.data.bGeoPoint['geopoint'].latitude,
                                                                                merchant.data.bGeoPoint['geopoint']
                                                                                    .longitude
                                                                            ),
                                                                            customerName: merchant.data.bName,
                                                                          )
                                                                ...
                                                                if(_formKey3.currentState.validate())
                                                                {
                                                                  ...
                                                                  var result = cancel(order.data.docID,
                                                                      Receipt.fromMap(order.data.receipt.copyWith(psStatus: "fail",
                                                                          pRef: '"${reason.value}" order was cancelled by ${widget.user.fname}(Logistic)').toMap()),
                                                                      widget.user.uid
                                                                  );
                                                                  if(result)
                                                                    await Utility.pushNotifier(fcm: customer.data.notificationID,
                                                                        title: 'Order Cancelled',
                                                                        body: 'Your order has been cancelled by the Rider (reason: ${reason.value}}). ${order.data.receipt.type != 'CASH'?'Please Note. your money has been refunded to your wallet':''}',
