...
class _MerchantUIState extends State<MerchantUI>{
  ...
  Widget _appBarTitle=new Text('widget.merchant.bName',style: TextStyle(color: PRIMARYCOLOR), );
  ViewModel vmodel;
  String barcode;
  int _value;
  ...
  void initState() {
    ...
    geoStream = location.onLocationChanged.listen((loc.LocationData cLoc) {
          position = Position(latitude: cLoc.latitude,
              longitude: cLoc.longitude,altitude: cLoc.altitude,
          accuracy: cLoc.accuracy);
          dist =  GeoFirePoint(
              cLoc.latitude,
             cLoc.longitude
          ).distance(lat: widget.merchant.bGeoPoint['geopoint'].latitude,
                          ...
                          Row(
                            ...
                            children: <Widget>[
                              ...
                              Row(
                                children: <Widget>[
                                  Badge(
                                    badgeContent: Text(cart.length.toString(),style: TextStyle(color:Colors.white),),
                                    ...
                                    showBadge: cart.length>0?true:false,
                                    animationDuration: Duration(seconds: 5),
                                  ),
                                  IconButton(icon: Icon(Icons.call,color: Colors.orangeAccent,),),
                                  IconButton(icon: Icon(Icons.place,color: Colors.orangeAccent,),),
                                ],
                              )
    ...
    if(cart.contains(item)){
      if(!Get.isSnackbarOpen)
        Get.snackbar(
            ...
            messageText: Text('Item already in the basket',style: TextStyle(color: Colors.white),),
      ...
      Get.snackbar(
          ...
          messageText: Text('Item added to the basket',style: TextStyle(color: Colors.white),),
          snackPosition: SnackPosition.TOP,
...
class OrderUI extends StatefulWidget{
  ...
  OrderUI({this.merchant,this.payload,this.user,this.distance,
    this.initPosition,this.cartOps});
  @override
...
class _OrderUIState extends State<OrderUI>{
    ...
    geoStream=location.onLocationChanged.listen((loc.LocationData cLoc) async{
        ...
        position = Position(latitude: cLoc.latitude,
         ...
         dist = await Geolocator().distanceBetween(
            cLoc.latitude, cLoc.longitude, widget.merchant.bGeoPoint['geopoint'].latitude,
            widget.merchant.bGeoPoint['geopoint'].longitude);
        if(mounted)
          setState(() {});
        deliveryETA(dist);
        deliveryCut(dist);
      await address(Position(latitude: cLoc.latitude,
          longitude: cLoc.longitude,altitude: cLoc.altitude,
          accuracy: cLoc.accuracy));
    });
    userChange=false;
  ...
  Future<void> address(Position position)async{
    final coordinates = new geocode.Coordinates(position.latitude,
        position.longitude);
  ...
  Widget stages(){
    switch(stage){
      case 'ORDERNOW':
        return widget.payload.runtimeType == List<CartItem>().runtimeType?stageOneOfList():stageOne();
      break;
      case 'FAROPTION':
        return stageTwo();
      break;
                      ...
                      child:Align(
                        ...
                        child: FlatButton(
                          ...
                          child: Container(
                            ...
                            child: Text('Clear Basket',style: TextStyle(color: Colors.black54),),
                          ),
                        )
                      ),
                  ),
                    ...
                    child: Align(
                        ...
                        child: FlatButton(
                          ...
                          child: Container(
                            ...
                            child: Text('Close Basket',style: TextStyle(color: Colors.black54),),
                          ),
                        )
                    ),
                  ),
            ...
            child: ListView.builder(
               ...
               Container(
                 ...
                 decoration: BoxDecoration(
                   ...
                   child:ListTile(
                 leading: Image.network(widget.payload[index].item.pPhoto.isNotEmpty? widget.payload[index].item.pPhoto[0]:PocketShoppingDefaultCover,
                 fit: BoxFit.fill),
                 title: Column(
                   children: <Widget>[
                     Align(
                       alignment: Alignment.centerLeft,
                       child: Text('${widget.payload[index].item.pName} @ \u20A6'
                           '${widget.payload[index].item.pPrice}',style:
                     ...
                     Row(
                       ...
                       children: <Widget>[
                         Expanded(
                           ...
                           child: Row(
                             children: <Widget>[
                               GestureDetector(
                                 onTap: (){
                                   if(widget.payload[index].count>1){
                                     widget.payload[index].count -=1;
                                     widget.payload[index].total = widget.payload[index].count * widget.payload[index].item.pPrice;
                                     setState(() { });
                                   }
                               ...
                               Container(
                                 ...
                                 child: Text('${widget.payload[index].count}'),
                               ),
                               GestureDetector(
                                 onTap: (){
                                   widget.payload[index].count +=1;
                         ...
                         Expanded(
                           flex: 0,
                           child: Text('\u20A6 ${widget.payload[index].total}'),
                         )
                       ],
                     )
                   ],
              ...
              child: Center(
                child: FlatButton(
                  onPressed: (){
                    ...
                    setState(() {
                      order = order.update(orderItem: OrderItem.fromCartList(widget.payload),
                      orderAmount: sum(widget.payload));
                      stage='FAROPTION';
                    });
                    },
                  child: Text('Next (\u20A6 ${sum(widget.payload)})',style: TextStyle(color: Colors.white),),
                ),
              ),
            ),
          ),
 ...
 Widget stageOne(){
          ...
          child: Column(
            children: <Widget>[
              SizedBox(
                  ...
                  width: MediaQuery.of(context).size.width,
                  child: Carousel(
                    images:widget.payload.pPhoto.isNotEmpty?List<NetworkImage>.generate(
                        widget.payload.pPhoto.length,
                            (int index) {
                          return NetworkImage(widget.payload.pPhoto[index]);
                        }
                    ).toList():[NetworkImage(PocketShoppingDefaultCover)],
                  )
              ),
              SizedBox(height: 10,),
                    ...
                    child: Container(
                      ...
                      child: FlatButton(
                        onPressed: (){
                          ...
                          setState(() {
                            order = order.update(orderItem: OrderItem.fromCartList([
                              CartItem(
                                  count: OrderCount,
                                  item: widget.payload,
                        ...
                        child: Text('Next (\u20A6 ${widget.payload.pPrice*OrderCount})',
            ...
            Column(
              children: <Widget>[
                Container(
                  ...
                  child: Text('How do you want it?',style: TextStyle(
                      fontSize: MediaQuery.of(context).size.height*0.03
                  ),),),
                ...
                ListTile(
                          ...
                          children: <Widget>[
                            ...
                            FlatButton(
                              onPressed: (){
                                ...
                                stage='OVERVIEW';
                                mode = 'Delivery';
                                order=order.update(orderMode: OrderMode(
                                  mode: mode,
                  ...
                  trailing: IconButton(
                    ...
                    icon: !homeDelivery?Icon(Icons.arrow_forward_ios):Icon(Icons.close),
                  ),
                ):
                ListTile(
                  leading:CircleAvatar(
                  ...
                  title: Text('Home delivery',style: TextStyle(
                      fontSize: MediaQuery.of(context).size.height*0.03,
                    color: Colors.grey
                  ),),
                  subtitle: Text('Sorry! we do not offer home delivery service',style: TextStyle(
                      color: Colors.grey
                  ),),
                ...
                :['Restuarant','Eatry','Bar'].contains(widget.merchant.bCategory)?
            Column(
              children: <Widget>[
                Container(
                  margin: EdgeInsets.only(bottom: 20),
                ...
                ListTile(
                  ...
                  title: Text('Take Away',style: TextStyle(
                      fontSize: MediaQuery.of(context).size.height*0.03
                  ),),
                  subtitle: Text('Choose this if you do not want to have you order here.'),
                  trailing: IconButton(
                    icon: Icon(Icons.arrow_forward_ios),
                  ),
                ),
                ...
                ListTile(
                  ...
                  title: Text('Serve Me',style: TextStyle(
                      fontSize: MediaQuery.of(context).size.height*0.03
                  ),),
                  subtitle: Column(
                    children: <Widget>[
                      Text('Choose this if you want to have you order here.'),
                      Form(
                        key: _tableformKey,
                        child: TextFormField(
                          controller: _table,
          ...
          Row(
            ...
            children: <Widget>[
              Padding(
                padding: EdgeInsets.symmetric(vertical: 10,horizontal: 10),
                child: IconButton(
                  onPressed: (){stage = 'FAROPTION'; setState(() { });},
                  icon: Icon(Icons.arrow_back),
                ),
              ),
              Padding(
                  ...
                  children: <Widget>[
                    ...
                    Align(
                      ...
                      child: Padding(
                        ...
                        child: Text('Item(s)',style: TextStyle(
                            fontSize: MediaQuery.of(context).size.height*0.025,
                    ...
                    Divider(height: 2,color: Colors.grey,),
                    Column(children: ItemMaker(widget.payload),),
                    Align(
                      alignment: Alignment.centerLeft,
                      child: Padding(
                        padding: EdgeInsets.symmetric(horizontal: 10),
                        child: Text('Details(s)',style: TextStyle(
                            fontSize: MediaQuery.of(context).size.height*0.025,
                            fontWeight: FontWeight.bold
                        ),),
                      ),
              ...
              child: Center(
                child: FlatButton(
                  onPressed: (){
                    ...
                    //stage = 'CHECKOUT';
                    setState(() { });},
                  child: Text('Checkout ( ${order.orderAmount} )',
                    style: TextStyle(color: Colors.white),),
                ),
              )
  ...
  List<Widget> ItemMaker(dynamic payload){
    if(payload.runtimeType == List<CartItem>().runtimeType){
      return List<Widget>.generate(payload.length, (index) =>
          Padding(
            padding: EdgeInsets.symmetric(vertical: 10,horizontal: 15),
            child: Column(
              children: <Widget>[
                Row(
                  ...
                  children: <Widget>[
                    Expanded(
                      child: Text(' ${payload[index].item.pName} @ ${payload[index].item.pPrice}'),
                    ),
                    Expanded(
                      child: Center(child: Text('${payload[index].count}'),),
                    ),
                    Expanded(
                      child: Center(child: Text('${payload[index].total}'),),
                    )
                  ],)
              ],
            ),
          ...
          children: <Widget>[
            Row(
              ...
              children: <Widget>[
                ...
                Expanded(
                  child: Center(child: Text('${payload.pPrice*OrderCount}'),),
                )
              ],)
          ],
        ),
            ...
            children: <Widget>[
              Row(
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: Center(child: Text('\u20A6${order.orderAmount}'),),
                  )
                ],),
              ...
              Row(
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: Center(child: Text('\u20A6${dCut+order.orderAmount}',style: TextStyle(fontWeight: FontWeight.bold),),),
                  )
                ],),
              Divider(height: 2,color: Colors.grey.withOpacity(0.5),),
              Row(
              ...
              Row(
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: Center(child: Text(_contact.text),),
                  )
                ],),
              Divider(height: 2,color: Colors.grey.withOpacity(0.5),),
              Row(
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: Center(child: Text(_address.text),),
                  )
                ],),
              Divider(height: 2,color: Colors.grey.withOpacity(0.5),),
              Row(
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: Center(child: Text('~$eta minute'),),
                  )
                ],),
            ...
            children: <Widget>[
              Row(
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: Center(child: Text('\u20A6${order.orderAmount}',style: TextStyle(fontWeight: FontWeight.bold),),),
                  )
                ],),
              Divider(height: 2,color: Colors.grey.withOpacity(0.5),),
              Row(
            ...
            children: <Widget>[
              ...
              Row(
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: Center(child: Text('${_table.text}'),),
                  )
                ],),
            ],
          ),
          ...
          Row(
            ...
            children: <Widget>[
              Padding(
                padding: EdgeInsets.symmetric(vertical: 10,horizontal: 10),
                child: IconButton(
                  onPressed: (){stage = 'OVERVIEW'; setState(() { });},
                  icon: Icon(Icons.arrow_back),
                ),
              ),
              Padding(
          ...
          Column(
            children: <Widget>[
              Container(
                padding: EdgeInsets.symmetric(vertical: 10,horizontal: 10),
                margin: EdgeInsets.only(bottom: 20),
                child: Text('Your order has been confirmed by ${widget.merchant.bName} proceed with payment to complete order',style: TextStyle(
                    fontSize: MediaQuery.of(context).size.height*0.025
                ,),textAlign: TextAlign.center,
                ),
              ),
              ...
              ListTile(
                ...
                title: Text('Pocketshopping',style: TextStyle(
                    fontSize: MediaQuery.of(context).size.height*0.03
                ),),
                subtitle: Column(
                  children: <Widget>[
              ...
              ListTile(
                ...
                title: Text('ATM Card',style: TextStyle(
                    fontSize: MediaQuery.of(context).size.height*0.03
                ),),
                subtitle: Text('Choose this if you want to pickup your order.'),
                trailing: IconButton(
   ...
   ProcessPay(dynamic method)async{
     ...
     if(reference.isNotEmpty){
       ...
       if(details != null){
         ...
         ).call({'data':order.toMap(),'lat':mode=='Delivery'?position.latitude:null,'long':mode=='Delivery'?position.longitude:null}).then((value) =>
         setState((){
           screenHeight=0.8;
           order.update(orderID: value.data.toString());
           paydone = true;
           startTimer();
         })
        ...
        Container(
            ...
            child: ListView(
                ...
                children: <Widget>[
                  ...
                  SizedBox(height: MediaQuery.of(context).size.height*formHeight,),
              ...
              Expanded(
                flex: 1,
                  child: Container(
                    child: Column(
                      children: <Widget>[
                        Text('Order successfully placed',style: TextStyle(
                          fontSize: MediaQuery.of(context).size.height*0.04
                        ),),
                        SizedBox(height: 10,),
                        Text('${_start}s',style: TextStyle(
  ...
  Future<http.Response> fetchPaymentDetail(String ref) async {
    ...
    final response = await http.get('https://api.paystack.co/transaction/verify/$ref', headers: {"Accept":"application/json",
    ...
    _timer = new Timer.periodic(
      oneSec,
          ...
          if (_start < 1) {
            ...
            Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => OrderTrackerWidget()));
          } else {
            _start = _start - 1;
          }
        },
          ...
          Center(
            child: TimerWidget(
              ...
              onFinish: (String accepted){
                ...
                stage = 'CHECKOUT';
                setState(() { });},
              onRepeat: ()async{ await sendAndRetrieveMessage();},
            ),
    ...
    await http.post('https://fcm.googleapis.com/fcm/send',
        ...
        body: jsonEncode(<String,dynamic>{
          'notification':<String,dynamic>{
            'body':'Hi. Can you process my order. ${order.orderCustomer.customerName}',
          ...
          'data':<String,dynamic>{
            ...
            'payload': {
              ...
              'Items':widget.payload.runtimeType==List<CartItem>().runtimeType?CartItem.toListMap(widget.payload):[{'productName':widget.payload.pName,'productCount':OrderCount}],
          ...
          'registration_ids':['fI8U-GHOSEKhEZXU3egwdU:APA91bEl8iZXRSNKsqcxzdfP9MYpwf52Bfht3Zk5e7tLDSgZ-u8yIHoYVVtK6qyl5S3-0BC01CnFBLBj5ZOD3jSVuWC1ni0uFWPQuoRki4HH0bD1Yg_OI8L9OLD4vCU0lIMwoRvA9X6f',],
        })
    );
  }
          ...
          children: <Widget>[
            ...
            Center(
              child: Padding(
                ...
                child: Text('Error processing payment. ensure you are entering the correct details',style: TextStyle(
                    fontSize: MediaQuery.of(context).size.height*0.025
                ),
                  textAlign: TextAlign.center,
                )
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Center(
              child: Padding(
                  padding: EdgeInsets.symmetric(vertical: 10),
                  child: Text('Placing Order please wait..',style: TextStyle(
                      fontSize: MediaQuery.of(context).size.height*0.025
                  ),
                    textAlign: TextAlign.center,
                  )
