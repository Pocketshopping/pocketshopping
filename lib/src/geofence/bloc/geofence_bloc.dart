...
class GeoFenceBloc extends Bloc<GeoFenceEvent, GeoFenceState> {
  ...
  Stream<GeoFenceState> _mapUpdateMerchantToState(List<Merchant> merchant)async*{
  ...
  Stream<GeoFenceState> _mapFetchNearByMerchantToState (Position _position,String category)async*{
    yield state.update(currentPosition: _position);
      ...
      var collectionReference = Firestore.instance.collection('merchants').where('businessCategory',isEqualTo: category).limit(10);
  ...
  Stream<GeoFenceState> _mapNearByMerchantToState(String category) async* {
    ...
    try {
      ...
      _positionSubscription = geolocator
          ...
          accuracy: LocationAccuracy.bestForNavigation, timeInterval: 30000))
          .listen((position) {
