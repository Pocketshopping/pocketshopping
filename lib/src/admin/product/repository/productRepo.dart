...
class ProductRepo {
  ...
  Future<String> save(
  {
    ...
    var bid=await databaseReference.collection("products")
        .add({
  ...
  Future<String> saveCategory(String pCategory,String mid)async {
    ...
    //if(cat.data.length>0){
      await databaseReference.collection("productsCategory").
      document(pCategory)
          .setData({
  ...
  Future<String> saveProductStock(
    ...
    String bid="";
    await databaseReference.collection("productStock")
        .add({
  ...
  Future<Map<String,dynamic>> getOne(String pID)async {
    var document = await Firestore.instance.collection('products').document(pID).get();
  ...
  Stream<int> getCount(String mID) {
    var businessRef = databaseReference.document('merchants/$mID');
     return Firestore.instance.collection('products')
        .where('productMerchant',isEqualTo:businessRef ).snapshots().map((element){
  ...
  Future<List<String>> getCategory(String pCategory)async {
    ...
    var document = await Firestore.instance.collection('productsCategory')
        .where('productCategory',isGreaterThanOrEqualTo:pCategory )
        .where('productCategory', isLessThan: pCategory + 'z').getDocuments();
  ...
  Future<List<Product>> fetchProduct(String mID,dynamic lastDoc,String category)async{
    ...
    if(lastDoc == null){
      document = await Firestore.instance.collection('products')
  ...
  Future<List<Product>> SearchProduct(String mID,dynamic lastDoc,String search)async{
    var mRef = Firestore.instance.document('merchants/$mID');
    List<Product> tmp=List();
    var document;
