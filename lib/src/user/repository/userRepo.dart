...
class UserRepo{
  ...
  Map<String,dynamic>makeData(
  {
    ...
    if(role != null && role.isNotEmpty)
      data['role']=role;
    if(notificationID == 'fcm')
      data['notificationID']=fcmToken;
  ...
  Stream<Session> getOne({String uid}) async*{
    ...
    var temp = await Firestore.instance.collection('merchants').where('businessCreator',isEqualTo:
    userRef).getDocuments();
      yield* Firestore.instance.collection('users').document(uid).snapshots().map((snapshot) {
      user = User.fromEntity(UserEntity.fromSnapshot(snapshot));

      if(temp.documents.length>0){
        merchant = Merchant.fromEntity(MerchantEntity.fromSnapshot(temp.documents[0]));
      }
  ...
  Stream<Merchant> getMyMerchant(DocumentReference mid) {
    ...
    return Firestore.instance.collection('merchants').document(mid.documentID).snapshots().map((merchantSnap){
