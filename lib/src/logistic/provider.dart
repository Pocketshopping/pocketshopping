...
class LogisticRepo {
  ...
  static Future<bool> activateDeactivateAgent(String agentID,
      {bool activate = true}) async {
    try {
      await updateAgent(agentID, {
        'agentStatus': activate ? 1 : 0,
      });
      await updateAgentLoc(agentID, {
        'parent': activate,
      });
  ...
  static Future<List<Agent>> fetchCompanyAgents(String company,
      {int source = 0}) async {
    try {
      try {
        ...
        document = await FirebaseFirestore.instance
            ...
            .timeout(Duration(seconds: CACHE_TIME_OUT), onTimeout: () {
          throw Exception;
        });
  ...
  static Future<List<AgentLocUp>> fetchMyAgents(String company, AgentLocUp last,
      {int source = 0, int count = 10}) async {
    try {
      QuerySnapshot document;
      if (last == null) {
        document = await FirebaseFirestore.instance
  ...
  static Future<List<AgentLocUp>> agentUpForTask(
      String company, AgentLocUp last,
      {int source = 0, int count = 10, String keyword = ''}) async {
  ...
  static Future<Map<String, AutoMobile>> fetchMyAutomobile(
      String company, AutoMobile last) async {
    QuerySnapshot document;
    if (last == null) {
      document = await FirebaseFirestore.instance
  ...
  static Future<bool> deleteAutomobile(String autoID,
      {String lid, int bCount = 0, int cCount = 0, int vCount = 0}) async {
    try {
  ...
  static Future<Map<String, AutoMobile>> searchMyAutomobile(
      String mID, AutoMobile last, String search,
      {int source = 0}) async {
    try {
      QuerySnapshot document;
      ...
      if (last == null) {
        ...
        try {
          ...
          document = await FirebaseFirestore.instance
              ...
              .where('index', arrayContains: search.toLowerCase())
              .startAfter(
                  [DateTime.parse(last.autoAddedAt.toDate().toString())])
              .limit(10)
              .get(GetOptions(source: Source.cache))
              .timeout(Duration(seconds: CACHE_TIME_OUT), onTimeout: () {
                throw Exception;
  ...
  static Future<List<AgentLocUp>> searchMyAgent(
      String mID, AgentLocUp last, String search,
      {int source = 0}) async {
    QuerySnapshot document;
  ...
  static Future<String> saveAutomobile(AutoMobile automobile,
      {int bCount = 0, int cCount = 0, int vCount = 0}) async {
  ...
  static Future<bool> saveAgent(
      Agent agent, Request request, AgentStatistic agentStatistic) async {
    try {
      DocumentReference bid;
      bid = await databaseReference
          .collection("agent")
      ...
      request =
          request.update(requestInitiatorID: bid.id, requestInitiator: 'agent');
      await RequestRepo.save(request);
      await StatisticRepo.updateAgentStatistic(agent.agent);
      await reAssignAutomobile(
        agent.autoAssigned,
        agent.agent,
      );
  ...
  static Future<bool> removeAgent(Agent agent,
      {String logistic, String fcm}) async {
    try {
      //Request request,
      ...
      if (isDeleted) {
        ...
        await reAssignAutomobile(agent.autoAssigned, 'Unassign',
            isAssigned: false);
        ...
        await Utility.updateWallet(
            uid: agent.agentWallet, cid: agent.workPlaceWallet, type: 2);
        await Utility.pushNotifier(
          fcm: fcm,
          body:
              'You are no longer a rider of $logistic. Contact $logistic for more information',
          title: '$logistic',
  ...
  static Future<bool> reAssignAutomobile(String autoID, String agent,
      {bool isAssigned = true}) async {
    try {
  ...
  static Future<bool> updateAgent(
      String agent, Map<String, dynamic> data) async {
    try {
      await databaseReference
          .collection("agent")
          .doc(agent)
  ...
  static Future<bool> updateAgentLoc(
      String agent, Map<String, dynamic> data) async {
    try {
      await databaseReference
          .collection("agentLocationUpdate")
  ...
  static Stream<AgentLocUp> getOneAgentLocationStream(String agentID) async* {
    yield* databaseReference
        ...
        .snapshots()
        .map((event) {
      return AgentLocUp.fromSnap(event);
    });
  }
  ...
  static Future<bool> revalidateAgentAllowance(
    String uid,
  ) async {
    try {
  ...
  static Future<bool> agentAccept(
      String agentID, String requestID, String uid) async {
    try {
      await updateAgent(agentID,
          {'agentStatus': 1, 'startDate': Timestamp.now(), 'accepted': true});
      ...
      await UserRepo()
          .upDate(role: 'rider', uid: uid, bid: agent.agentWorkPlace);
      await UserRepository().changeRole('rider');
      await Utility.updateWallet(
          uid: agentAcct.walletId, cid: agent.workPlaceWallet, type: 5);
      await LocRepo.update({
        'agentParent': agent.agentWorkPlace,
        'wallet': agent.agentWallet,
  ...
  static Future<List<String>> getNearByAgent(Position mpos, String autoType,
      {bool isDevice = true}) async {
  ...
  static Future<bool> remittance(String wid,
      {int limit = 10000, String key = ''}) async {
    try {
      var remit =
          await Utility.generateRemittance(aid: wid, limit: limit, key: key);
      //print(remit);
      if (remit['remittance']) {
        return false;
      } else {
  ...
  static Future<Map<String, dynamic>> getRemittance(String wid,
