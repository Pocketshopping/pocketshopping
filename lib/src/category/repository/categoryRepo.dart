...
class CategoryRepo {
  ...
  static Future<List<MCategory>> getAll()async{
    ...
    var docs = await databaseReference.collection("categories").orderBy('categoryView',descending: true).getDocuments();
    docs.documents.forEach((element) {categories.add(MCategory.fromEntity(
        MCategoryEntity.fromSnapshot(element)
    ));});
    return categories;
  }
  ...
  static Future<List<String>> categoria()async{
    ...
    categories.forEach((element) {cname.add(element.categoryName);});
    return cname;
  }

  static Future<List<String>> deliveria()async{
    List<String>options =List();
    var docs = await databaseReference.collection("delivery").orderBy('sort').where('enable',isEqualTo: true).getDocuments();
    docs.documents.forEach((element) {options.add(element.data['key']);});
    return options;
  }
