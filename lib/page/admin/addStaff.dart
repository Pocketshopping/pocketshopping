...
class _AddStaffState extends State<AddStaff> {
  ...
  setBranches(){
    if (psProvider
      ...
      if (psProvider
        ...
        MerchantDataModel().getBranch(psProvider
            .of(context)
            .value['user']['merchantID']).
        then((result) =>
        {
          ...
          bData[psProvider
              .of(context)
              .value['user']['businessName'].toString()]= psProvider
              .of(context)
              .value['user']['merchantID'],
          result.forEach((doc) {
            ...
            bData[
                doc.data['businessName'].toString() +
                    ' (${doc.data['branchUnique'].toString()})']= doc.documentID;
          }),
          //office=branches[0],
          setState(() {})
        });
  ...
  Widget build(BuildContext context) {
    ...
    final manager = CheckboxListTile(
      title: Text("Manage Staff,Business setttings and overall business, this Staff can add new stafffs, edit business settings and perform all function excluding special privilege which are reserve for business owner. staff with this privilege can serve as manager   etc."),
      value: this.managers,
      onChanged: (bool value) {
        setState(() {
          this.managers = value;
        ...
        onPressed: ()async {
          if (_formKey.currentState.validate()) {
            if(!loaded){
              ...
              if(data.isNotEmpty){
                if(_psidController.text == psProvider.of(context).value['user']['email']){
                  setState(() {
                    report="You own the business";
                  });
                }
                else if(data['business'].toString().isNotEmpty && data['role'].toString()=='admin' ){
                  setState(() {
              ...
              if(permissions.containsValue(true)){
                ...
                print(bData[office]);
                  ...
                  content: Text('${sData['fname']} as been sent a request! Once accepted will be added as a staff'),
                ...
                Scaffold.of(nctx).showSnackBar(SnackBar(
                  content: Text('You have to select atleast one permission to add staff '),
                  behavior: SnackBarBehavior.floating,
                ));
              }
            }
                                    ...
                                    Column(
                                      children: <Widget>[
                                        Container(
                                          ...
                                          child: Text('Enter email of new staff! Note user email must be registered on pockshopping'),
                                        ),
                                        Container(
                                          decoration: BoxDecoration(
                                            border: Border(
