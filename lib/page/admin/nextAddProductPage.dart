...
class _AddProductNextPageState extends State<AddProductNextPage> {
  ...
  Widget build(BuildContext context) {
    final mDate = FlatButton(
        ...
        onPressed: (){
          showMonthPicker(context: context,
              ...
              initialDate: expiryDate??initialDate).then((date) =>
          {
            if(date != null){
              setState((){
                manufactureDate=date;
                manufactureText=date.month.toString()+"/"+date.year.toString();
        ...
        child: Row(children: <Widget>[
          Expanded(
            child:
            Text(manufactureText,style:TextStyle(color: Colors.black54,fontSize: 16),),
          ),
        ],)
    );
    ...
    final eDate =  FlatButton(
      ...
      onPressed: (){
            ...
            initialDate: expiryDate??initialDate).then((date) =>
        {
          if(date != null){
            setState((){
              expiryDate=date;
              expiryText=date.month.toString()+"/"+date.year.toString();
      ...
      Expanded(
        child:
        Text(expiryText,style:TextStyle(color: Colors.black54,fontSize: 16),),
      ),
      ],)
    );
                            ...
                            child: Column(
                              children: <Widget>[
                                Align(
                                  ...
                                  child: Text(mDateLabel,style: TextStyle(color: Colors.black54),),
                                ),
                                mDate
                              ],
                            ),
                          ...
                          Container(
                            ...
                            child: Column(
                              children: <Widget>[
                                Align(
                                  alignment: Alignment.centerLeft,
                                  child: Text(eDateLabel,style: TextStyle(color: Colors.black54),),
                                ),
                                eDate
                              ],
                            ),
                            ...
                            child: Column(
                              children: <Widget>[
                                ...
                                Container(
                                  ...
                                  child: barcode.isNotEmpty?Text(barcode):Container(),
                                )
                              ],
                            ),
                          ),
                          Container(
                            ...
                            child: Padding(
                              ...
                              child: FlatButton(
                                  ...
                                  color: PRIMARYCOLOR,
                                  onPressed: () async{
                                    FocusScope.of(context).requestFocus(this._submitFocus);
                                    if (_formKey.currentState.validate()) {
                                      if(!submitted){
                                        setState(() {
                                          submitted=true;
                                          report='Creating Product..please wait';
                                        });
                                        widget.product.pUnit = _unitController.text.trim()??"";
                                        widget.product.pStockCount = int.parse(_stockController.text.isNotEmpty?_stockController.text:"0");
                                        widget.product.pManufacturer=_manufacturerController.text.trim()??"";
                                        widget.product.pMDate=manufactureText != 'Manufactured Date(optional)'?manufactureText:"";
                                        widget.product.pEDate=expiryText != 'Expiry Date(optional)'?expiryText:"";
                                        widget.product.pQRCode=barcode??"";
                                        widget.product.pGroup=psProvider.of(context).value['user']['businessCategory'];
                                        widget.product.pPhoto = await uploadMultipleImages(widget.product.pFilePhoto);
                                        String pid = await widget.product.save();
                                        if(pid.isNotEmpty){
                                          Navigator.push(
                                              context,
                                              MaterialPageRoute(
                                                  builder: (context) =>AddProduct(withSuccess: true,)));
                                        }
                                  ...
                                  child: !submitted?Center(child:Text("Create",style: TextStyle(color: Colors.white),))
                                      :Row(
  ...
  Future<List<String> > uploadMultipleImages(List<File> _imageList) async {
    ...
    try {
      for (int i = 0; i < _imageList.length; i++) {
        final StorageReference storageReference = FirebaseStorage().ref().child("ProductPhoto/${DateTime.now()}.png");
